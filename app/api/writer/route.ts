...
function postProcess(raw: string, n: NormalizedInput): string {
  ...
  const normalizeQ = (s: string) =>
    s.replace(/^[\s\d\.\):：）\-・]+/, "").replace(/[？?\s]+$/g, "").replace(/\s+/g, " ").toLowerCase();
  ...
  let dedup = Array.from(dedupMap.values());

  const priority = [/(返品|返金|保証)/, /(対応|互換|相性)/, /(配送|送料|納期|到着|何日)/];
  dedup.sort((a, b) => {
  ...
  /* ---- 共起語 濃度上限 & 表示モード ----
     WRITER_COOC_MAX: 追加する上限（既定 3）
     WRITER_FOOTNOTE_MODE: "inline" | "compact" | "none"（既定 "compact"）
  */
  ...
  const safety2 = lex.safetyPhrases[1] ?? "";

  if (picked.length > 0 || safety1) {
    if (footnoteMode === "none") {
      // 何も追加しない（CTAの利益行は後段で保証）
    } else if (footnoteMode === "inline") {
      // CTA行に安心要素を織り込み。別行の脚注は出さない。
      // 置換は後段の CTA 統一フェーズで同時に行われるため、ここではフラグとして保持のみ。
      // → 実装簡潔化のため、CTA統一フェーズで「—」後ろに safety1 を優先追加。
