...
const DUR = {
  ...
  DONE_BADGE_MS: 3600, // 完了バッジ
  FLASH_RING_MS: 3200, // 淡い光（オーラ）
  CELEB_MS: 3800,      // 完成演出全体（justCompleted寿命）
};
  ...
  const onSubmit = useCallback(
    async (vals: FormValues) => {
      ...
      try {
        ...
        setTimeout(() => setJustCompleted(false), DUR.CELEB_MS);
  ...
  return (
    <div className={TOKENS.pageBg}>
      ...
      <div className="mx-auto max-w-7xl px-8 md:px-12 mt-3">
        <div className="flex items中心 gap-2 text-[12px] text-neutral-600">
          <span className="inline-flex items-center gap-1 rounded-full border px-2 py-1 bg-white/70">
      ...
      <div className="mx-auto max-w-7xl px-8 md:px-12 py-6 grid grid-cols-1 lg:grid-cols-[1.1fr,0.9fr] gap-8">
        ...
        <motion.section
          ...
          className={clsx(
            TOKENS.outputWrap,
            "p-5 md:p-6 overflow-visible", // クリッピング回避
            justCompleted &&
        ...
        >
          ...
          {/* 完成演出オーバーレイ（justCompleted 中のみ） */}
          <AnimatePresence initial={false}>
            {justCompleted && !isLoading && !error && (
              <div className="pointer-events-none absolute inset-0 z-50 overflow-visible">
                {/* 淡い光（オーラ） */}
                <motion.div
                  className="absolute -inset-2 rounded-[16px] ring-2 ring-indigo-300/50"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: [0, 0.9, 0.45, 0.65, 0] }}
                  exit={{ opacity: 0 }}
